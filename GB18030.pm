...
END
        ...
        if (s/^ \s* use \s+ GB18030 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list !~ m/\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Egb18030::ord'     if m/\A ord \z/oxms;
                    $function_ord_    = 'Egb18030::ord_'    if m/\A ord \z/oxms;
                    $function_reverse = 'Egb18030::reverse' if m/\A reverse \z/oxms;
                }
            }
        }
    }
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            ...
            $e_tr = qq{Egb18030::tr($variable,$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_s2 {
    ...
    if ($left_e > $right_e) {
        if ($modifier =~ /e/) {
            return join '',
                $delimiter,
                'Egb18030::shift_matched_var().',     @char, '>]}' x ($left_e - $right_e),
        ...
        if ($modifier =~ /e/) {
            return join '',
                $delimiter,
                'Egb18030::shift_matched_var().',     @char,
                $end_delimiter, $modifier;
        }
        else {
            return join '',
...
the script was written in GB18030.
...
we're loving, so many users hope to write scripts in GB18030 encodings; without
giving up a whole new feature of Perl5.8, Perl5.10.

The GB18030 was developed in order to maintain backward compatibility. Generally,

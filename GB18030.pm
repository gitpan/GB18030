...
sub e_qr {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', $ope, $delimiter, "$your_gap(?:", @char, '>]}' x ($left_e - $right_e), ')(?{Egb18030::m_matched})', $end_delimiter, $modifier;
    }
...
sub e_s1 {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', $ope, $delimiter, "($your_gap)(?:", @char, '>]}' x ($left_e - $right_e), ')(?{Egb18030::s_matched})', $end_delimiter, $modifier;
    }
...
Egb18030::glob provides a portable enhanced DOS-like globbing for the GB18030 software.
Egb18030::glob lets you use wildcards in directory paths, is case-insensitive, and
